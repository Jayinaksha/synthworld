...
class WorldGenerator:
    ...
    def _generate_chunk_buildings(self, center_x: float, center_y: float,
        ...
        if urban_ratio > 0.3:
            ...
            buildings = self.building_gen.generate_city_block(
                ...
                building_density=0.4 + urban_ratio * 0.4
            )
        ...
        if urban_ratio > 0.3:
            ...
            for _ in range(num_lots):
                ...
                props.extend(self.prop_gen.generate_parking_lot(
                    lot_x, lot_y, 20, 30, fill_ratio=0.5
                ))
    ...
    def load_chunk(self, chunk_x: int, chunk_y: int):
        ...
        def color_func(height):
            # Simple cyberpunk color based on height
            if height < 0:
                return (0.05, 0.1, 0.2, 0.8)  # Water - dark blue
            elif height < 1:
                return (0.15, 0.15, 0.18, 1.0)  # Urban - concrete
            else:
                return (0.2, 0.18, 0.15, 1.0)  # Elevated - rusty
        ...
        for building in chunk.buildings:
            if self._renderer:
                node = create_building_node_path(building, self._renderer.buildings_node)
                building.node_path = node
        
        # Note: Props would need similar treatment
