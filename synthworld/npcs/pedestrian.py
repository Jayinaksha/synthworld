...
class Pedestrian(NPC):
    ...
    def spawn(self, position: Tuple[float, float, float],
        ...
        if self._physics:
            ...
            # Create cylinder for pedestrian (simplified shape)
    ...
    def _create_visual(self):
        ...
        # Create a simple person shape using available primitives
        height = self.appearance.height
        
        # Body (simple box/cylinder)
        try:
            # Try to create a simple representation
            body = self._renderer.create_cylinder(
                radius=0.2,
                height=height * 0.6,
            ...
            logger.warning(f"Could not create pedestrian visual: {e}")
            return None
