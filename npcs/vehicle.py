...
class VehicleNPC(NPC):
    ...
    def _create_visual(self):
        ...
        # Cabin
        cabin = self._renderer.create_box(
            s.width * 0.9, s.length * 0.5, s.height * 0.4,
            color=(0.1, 0.1, 0.15, 0.8),  # Dark tinted
            position=(0, -s.length * 0.1, s.height * 0.5)
        )
        cabin.reparentTo(body)
        
        # Wheels (simplified as boxes)
        wheel_positions = [
            (-s.width/2, s.length/3, -s.height/3),
            (s.width/2, s.length/3, -s.height/3),
            (-s.width/2, -s.length/3, -s.height/3),
            (s.width/2, -s.length/3, -s.height/3),
        ]
        ...
        for wp in wheel_positions:
            wheel = self._renderer.create_cylinder(
                0.3, 0.2,
                color=(0.15, 0.15, 0.15, 1.0),
                position=wp,
                rotation=(90, 0, 0)
            )
            wheel.reparentTo(body)
        
        # Headlights
        for x_offset in [-s.width/3, s.width/3]:
            light = self._renderer.create_sphere(
                0.1,
                color=(1.0, 1.0, 0.8, 1.0),
                position=(x_offset, s.length/2, 0)
            )
            light.reparentTo(body)
